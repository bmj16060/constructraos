...
- The repo-backed project contract now includes first-class environment lifecycle records and workflow-side policy checks for environment launch/reuse decisions, so namespace ownership and cleanup can start from durable state instead of hidden workflow state.
